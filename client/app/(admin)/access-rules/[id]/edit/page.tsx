...
import { UpdateAccessRulePayload } from "@/types/form.types";
import api from "@/config/axios";
import { API_CONTS } from "@/lib/api";
import { firebaseService } from "@/lib/firebaseService";

const editAccessRuleSchema = z.object({
...
type EditAccessRuleInput = z.infer<typeof editAccessRuleSchema>;
...
export default function EditAccessRulePage() {
    ...
    const {
        ...
        reset,
        watch,
    } = useForm<EditAccessRuleInput>({
        resolver: zodResolver(editAccessRuleSchema),
    });
        ...
        const fetchRuleData = async () => {
            ...
            try {
                ...
                reset({
                    ...
                    allowed_user_group_id: data.allowed_user_group_id || 4, // default if null to prevent blank inputs, or keep null mapped
    ...
    const onSubmit = async (data: EditAccessRuleInput) => {
        setIsSaving(true);
        try {
            const payload: UpdateAccessRulePayload = {
                ...
                allowed_user_group_id: data.allowed_user_group_id || null, // Allow removing user groups
                otp_required: data.otp_required,
            };

            await accessRuleService.updateAccessRule(Number(id), payload);
            toast.success("Access rule updated successfully");
            router.push("/access-rules");
        } catch (error: any) {
            console.error("Error updating rule:", error);
            const errMsg = error.response?.data?.detail || "Failed to update access rule.";
            toast.error(errMsg);
        } finally {
            setIsSaving(false);
        }
    ...
    if (isLoading) {
        return (
            <div className="flex h-[400px] w-full flex-col items-center justify-center gap-4">
                ...
                <p className="text-muted-foreground">Loading rule configuration...</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Edit Access Rule</h1>
                    <p className="text-muted-foreground">
                        Modify security restrictions for this rule configuration.
                    </p>
                </div>
            </div>
            ...
            <div className="mx-auto w-full max-w-2xl">
                <Card className="border-t-4 border-t-primary shadow-md">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Edit email, group, and multi-factor constraints for rule #{id}.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => setValue("form", Number(value), { shouldValidate: true, shouldDirty: true })}
                                ...
                                <div className="flex flex-row items-center justify-between rounded-lg border p-4 bg-muted/10 shadow-sm">
                                    ...
                                    <div className="flex items-center space-x-2 mr-4">
                                        <Switch
                                            ...
                                            onCheckedChange={(checked) => setValue("otp_required", checked, { shouldValidate: true, shouldDirty: true })}
                                            disabled={isSaving}
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-3 pt-4 border-t">
                                ...
                                <Button type="submit" disabled={isSaving || !isDirty || isLoadingForms} className="flex-1">
                                    {isSaving ? (
                                        <>
