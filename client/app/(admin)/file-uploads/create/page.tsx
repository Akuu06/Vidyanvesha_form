...
type CreateFileUploadInput = z.infer<typeof createFileUploadSchema>;

export default function CreateFileUploadPage() {
    ...
    const {
        ...
        setValue,
        watch,
    } = useForm<CreateFileUploadInput>({
        resolver: zodResolver(createFileUploadSchema),
        defaultValues: {
            file_path: "uploads/2026/03/"
        }
    });
    ...
    const onSubmit = async (data: CreateFileUploadInput) => {
        setIsSaving(true);
        try {
            await fileUploadService.createFileUpload(data);
            toast.success("File upload record created successfully");
            router.push("/file-uploads");
        } catch (error: any) {
            console.error("Error creating record:", error);
            const errMsg = error.response?.data?.detail || "Failed to create file upload record.";
            toast.error(errMsg);
        } finally {
            setIsSaving(false);
        }
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Create File Upload Record</h1>
                    <p className="text-muted-foreground">
                        Register a new file upload directly to a specific response and question.
                    </p>
                </div>
            </div>
            ...
            <div className="mx-auto w-full max-w-2xl">
                <Card className="border-t-4 border-t-primary shadow-md">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Link the file path with its corresponding form entities.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="space-y-4 p-4 border rounded-lg bg-muted/20">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        onValueChange={(value) => setValue("response", Number(value), { shouldValidate: true })}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        onValueChange={(value) => setValue("question_id", Number(value), { shouldValidate: true })}
                                        disabled={isSaving || isLoadingData}
                                    >
                            ...
                            <div className="flex gap-3 pt-4 border-t">
                                ...
                                <Button type="submit" disabled={isSaving || isLoadingData} className="flex-1">
                                    {isSaving ? (
                                        <>
